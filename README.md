...
**Background processing** relies on a Vercel Cron job (configured in [vercel.json](vercel.json), hitting `/api/cron/process-pending` every minute) to guarantee large imports eventually finish — this only runs on Vercel, not in local dev. Locally, a normal-sized import still processes fully via the inline pass that kicks off right after upload; only a very large import would need the cron sweep to finish the rest, which won't happen locally.
...
- **Import processing runs via `after()` right after upload, with a Vercel Cron sweep as the safety net.** A serverless invocation's lifetime is finite even when extended to run work after the response is sent, so a very large import might not finish inline. A cron job (every minute) claims anything still `pending` and keeps going — an import always finishes eventually, whether or not it fit in the first pass. Claiming uses `UPDATE ... WHERE id IN (SELECT ... FOR UPDATE SKIP LOCKED)`, the standard atomic-claim pattern, so the inline pass and the cron sweep can never double-process the same row.
